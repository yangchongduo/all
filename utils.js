...
var utils = (function () {
    ...
    function offset(curEle) {
        ...
        while (par) {
            ...
            if (navigator.userAgent.indexOf('MSIE 8.0') === -1) {//高级浏览器执行
                l += par.clientLeft;
                t += par.clientTop;
            }
            l += par.offsetLeft;
    ...
    /**
     *
     * @param curEle  通过className获取元素  curEle下的标签集合
     * @param strClass  一个className或者多个
     * @returns {*}  返回一个数组
     */
    function getByClass(curEle, strClass) {
        ...
        var ary = [];
        var aryClass = strClass.replace(/(^\s+)|(\s+$)/g, '').split(/\s+/g);//replace是为了去除首位的空格
    ...
    function hasClass(curEle, strClass) {
        var reg = new RegExp('(\\b)' + strClass + '(\\b)');//实例创建的正则，特殊意义的元字符需要转义
    ...
    //function children(curEle, tagName) {//张蕾版
    //    var ary = [];
    //    if (flag) {
    ...
    //    return ary;
    //}
    function getChildren(ele, tagName) {//王燕版
